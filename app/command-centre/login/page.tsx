...
export const metadata: Metadata = {
  title: "Command Centre sign in",
  robots: { index: false, follow: false },
};

export default async function CccLoginPage({
  searchParams,
}: {
  ...
  const from =
    params.from && params.from.startsWith("/command-centre")
      ? params.from
      : "/command-centre";
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-white/10 bg-forest-dark/60 p-8 shadow-[0_24px_60px_rgba(0,0,0,0.35)] backdrop-blur-sm">
        <div className="mb-6 text-center">
          <div className="mb-3 inline-flex items-center gap-2 rounded-full border-2 border-gold/50 bg-white/10 px-3 py-1 font-caveat text-[1rem] font-bold text-gold-soft">
            <Paw className="h-[1.1em] w-[1.1em]" /> Genera Content
          </div>
          <h1 className="font-massilia text-2xl font-bold text-white">
            Command Centre
          </h1>
          <p className="mt-1 text-sm text-white/70">
            Shared with Duncan. Same password each time.
          </p>
        </div>
        <LoginForm from={from} />
      </div>
