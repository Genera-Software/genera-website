...
//
// Deliberately a SEPARATE Supabase project from the main site (which uses
// NEXT_PUBLIC_SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY): this one is the same
// project the CRM (public/crm/index.html) already runs on, since that's the
// Supabase account Jess actually has access to. Do not merge these two.
//
...
function getSupabase() {
  const url = process.env.CCC_SUPABASE_URL;
  const key = process.env.CCC_SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !key) {
    throw new Error(
      "Missing CCC_SUPABASE_URL or CCC_SUPABASE_SERVICE_ROLE_KEY env vars",
    );
  }
